  ...
  return (
    <section className="animate-rise rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">
        All candidate keywords ({keywords.length})
      </h2>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-2 text-right font-medium">URL freq</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {keywords.map((k, i) => (
              <tr key={`${k.keyword}-${i}`}>
                ...
                <td className="px-4 py-2 text-right text-slate-600">{k.urlFrequency}</td>
                <td className="px-4 py-2 text-right text-slate-600">{formatVolume(k.volume)}</td>
                <td className="px-4 py-2 text-right text-slate-600">{k.compositeScore}</td>
              </tr>
            ))}
          </tbody>
        </table>
